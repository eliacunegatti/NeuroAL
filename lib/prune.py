...
def prune_magnitude(
    args, model, tokenizer, device=torch.device("cuda:0"), dist=None, granularity='row'):
    if granularity == None:
        granularity = 'row'
...
def prune_wanda(args, model, tokenizer, device=torch.device("cuda:0"), dataloader=None, dist=None, granularity='row'):
...
@torch.no_grad()
def prune_sparsegpt(args, model, tokenizer, dev, dataloader=None, dist=None):
...
def prune_DSnoT(
    args, model, tokenizer, device=torch.device("cuda:0"), dataloader=None, prune_n=0, prune_m=0
):
    use_cache = model.config.use_cache
    model.config.use_cache = False
    ...
    for i in range(len(layers)):
        ...
        for name in subset:
            wrapped_layers[name] = WrappedGPT(
                subset[name],
                initial_method=args.prune_method
            )
...
def prune_multiflow(
    args, model, tokenizer, device=torch.device("cuda:0"), dataloader=None, dist=None, granularity='row'):
    use_cache = model.config.use_cache
    model.config.use_cache = False
    if granularity == None:
        granularity = 'row'

    #distribution = get_global_distribution(model, args.sparsity_ratio, device)
