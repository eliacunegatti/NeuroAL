...
def prune_magnitude_opt(args, model, tokenizer, device=torch.device("cuda:0"), dist=None, granularity='row'):
    if granularity == None:
        granularity = 'row'
    ...
    for i in range(len(layers)):
        ...
        for name in subset:
            ...
            if dist == None:
                W_mask = compute_mask(W_metric, granularity, args.sparsity_ratio)
            else:
                W_mask = compute_mask(W_metric, granularity, dist[f'{name}-{i}'])
            W[W_mask] = 0
    return None


def prune_wanda_opt(args, model, tokenizer, device=torch.device("cuda:0"), dataloader=None, dist=None, granularity='row'):
...
def prune_multiflow_opt(args, model, tokenizer, device=torch.device("cuda:0"), dataloader=None, dist=None, granularity='row'):
    if granularity == None:
        granularity = 'row'
    use_cache = model.config.use_cache
    model.config.use_cache = False
    ...
    for i in range(len(layers)):
        ...
        for name in subset:
            ...
            actn_norm = torch.sqrt(wrapped_layers[name].scaler_row.reshape((1,-1)))
            importance_per_output = (W_metric * actn_norm).mean(dim=1)
            importance_per_input = (W_metric * actn_norm).mean(dim=0)

            W_metric *= torch.ger(importance_per_output, importance_per_input)
            if dist == None:
                W_mask = compute_mask(W_metric, granularity, args.sparsity_ratio)
            else:
                W_mask = compute_mask(W_metric, granularity, dist[f'{name}-{i}'])
...
@torch.no_grad()
def prune_sparsegpt_opt(args, model, tokenizer, dev, dataloader, dist=None):
...
def prune_DSnoT_opt(args, model, tokenizer, device=torch.device("cuda:0"), dataloader=None, prune_n=0, prune_m=0):
    use_cache = model.config.use_cache 
    model.config.use_cache = False 
    ...
    for i in range(len(layers)):
        ...
        wrapped_layers = {}
        for name in subset:
            wrapped_layers[name] = WrappedGPT(subset[name], initial_method=args.prune_method)
        ...
        for name in subset:
            ...
            if args.prune_method == "wanda":
                ...
                initial_metric = torch.abs(subset[name].weight.data) *  torch.ger(importance_per_output, importance_per_input)
            ...
            if prune_n != 0:
                ...
                if (name.startswith("self_attn") and args.skip_layer=="self_attn") or (not name.startswith("self_attn") and args.skip_layer=="fc") or (name == args.skip_sub_layer) or args.without_DSnoT:
